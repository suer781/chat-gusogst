...
export function matchPlatform(userInput: string): ConnectFlow | null {
  ...
  if (input.includes('wechat') || input.includes('weixin') || input.includes('\u5fae\u4fe1')) {
    return CONNECT_FLOWS.weixin
  }
  if (input.includes('qq')) {
    return CONNECT_FLOWS.qq
  }
  if (input.includes('telegram') || input.includes('tg') || input.includes('\u7535\u62a5')) {
    return CONNECT_FLOWS.telegram
  }
  if (input.includes('feishu') || input.includes('lark') || input.includes('\u98de\u4e66')) {
    return CONNECT_FLOWS.feishu
  }
  if (input.includes('discord') || input.includes('dc')) {
    return CONNECT_FLOWS.discord
  }
  if (input.includes('dingtalk') || input.includes('\u9489\u9489')) {
    return CONNECT_FLOWS.dingtalk
  }
