    ...
    <div className="h-full flex flex-col" style={{ background: '#0f0f23' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #1a1a3a' }}>
        <div className="flex items-center justify-between">
          <div>
            <div style={{ fontSize: 14, fontWeight: 600 }}>{providers.length}{t('provider.providers')} · {totalModels}{t('provider.models')}</div>
            <div style={{ fontSize: 11, color: '#666688' }}>{t('provider.dataFrom')}</div>
          </div>
          {currentModel.model && <div style={{ fontSize: 12, color: '#e94560', fontWeight: 500 }}>{currentModel.model}</div>}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto" style={{ padding: '8px 16px' }}>
              ...
              <button onClick={() => { setExpandedId(expanded ? null : p.id); setModelSearch('') }} className="w-full flex items-center gap-3" style={{ padding: '12px 16px', background: 'none', border: 'none', cursor: 'pointer', color: '#e0e0e0', textAlign: 'left' }}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {hasKey && <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#22c55e' }} />}
                  </div>
                  <span style={{ fontSize: 12, color: '#666688' }}>{p.models.length} {t('provider.models').replace('个', '')}</span>
                </div>
                <ChevronRight size={16} style={{ color: '#666688', transform: expanded ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s' }} />
              </button>
              {expanded && (
                <div style={{ borderTop: '1px solid #2a2a4a', padding: '12px 16px' }}>
                  ...
                  <div className="flex items-center gap-2" style={{ marginBottom: 8 }}>
                    <button onClick={() => fetchLive(p)} disabled={fetching === p.id} style={{ background: '#e9456015', border: 'none', borderRadius: 8, padding: '6px 12px', cursor: 'pointer', color: '#e94560', fontSize: 12, fontWeight: 500 }}>
                      <RefreshCw size={12} className={fetching === p.id ? 'animate-spin' : ''} style={{ display: 'inline', marginRight: 4 }} /> {t('provider.fetchLive')}
                    </button>
                    {connected[p.id] === true && <Check size={14} style={{ color: '#22c55e' }} />}
                  ...
                  <div style={{ maxHeight: 250, overflowY: 'auto' }}>
                    {getDisplayModels(p).map((m) => {
                      ...
                      return (
                        <button key={m.id} onClick={() => setModel(p.id, m.id, apiKeys[p.id] || '', baseUrls[p.id] || '')} className="w-full flex items-center gap-2" style={{ background: active ? '#e9456015' : 'transparent', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', color: active ? '#e94560' : '#e0e0e0', textAlign: 'left', fontSize: 13 }}>
                          <span className="flex-1 truncate">{m.name || m.id}</span>
                          {m.context_length > 0 && <span style={{ fontSize: 11, color: '#666688' }}>{fmtCtx(m.context_length)}</span>}
                          {m.cost_input > 0 && <span style={{ fontSize: 11, color: '#22c55e80' }}>{fmtCost(m.cost_input)}</span>}
        ...
        {filtered.length === 0 && <div className="text-center" style={{ padding: '48px 0', color: '#666688', fontSize: 14 }}>{t('provider.noMatch')}</div>}
      </div>
    </div>
  )
}
