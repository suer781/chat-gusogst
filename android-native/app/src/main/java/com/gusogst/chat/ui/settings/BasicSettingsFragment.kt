            ...
            for ((key, label, icon) in themeOptions) {
                ...
                grid.addView(LinearLayout(requireContext()).apply {
                    ...
                    background = GradientDrawable().apply {
                        setColor(if (isActive) ContextCompat.getColor(requireContext(), R.color.accent_soft_26) else ContextCompat.getColor(requireContext(), R.color.overlay_light_0A))
                        setStroke(if (isActive) 2 else 1, if (isActive) ContextCompat.getColor(requireContext(), R.color.accent_soft_80) else resources.getColor(R.color.transparent, null))
