...
async function searchTavily(query: string, apiKey: string, maxResults: number): Promise<string> {
  ...
  const results = (data.results || []).map((r: any) => ({ title: r.title, url: r.url, snippet: r.content?.slice(0, 200) }))
...
async function searchDuckDuckGo(query: string, maxResults: number): Promise<string> {
  ...
  if (data.Abstract) results.push({ title: data.Heading || query, snippet: data.Abstract })
  for (const topic of (data.RelatedTopics || []).slice(0, maxResults)) {
    if (topic.Text) results.push({ title: topic.Text?.slice(0, 80), snippet: topic.Text })
  }
