...
export function ProviderSettings({ onDone }: { onDone: () => void }) {
  ...
  const { currentModel, setModel } = useSettingsStore();
  ...
  const Card = (p: ProviderInfo) => {
    ...
    const active    = currentModel?.providerId === p.id;
      ...
      <div key={p.id} style={{
        ...
        {open && <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--border-light, rgba(0,0,0,0.05))' }}>
          ...
          <div style={{ maxHeight: 250, overflowY: 'auto', borderRadius: 8 }}>
            {models.map(m => {
              const sel = currentModel?.modelId===m.id && currentModel?.providerId===p.id;
              return (
                <button key={m.id}
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg-primary, #f0f2f5)' }}>
      ...
      {currentModel && <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--accent, #6C5CE7)' }}>
          {currentModel.modelName || currentModel.modelId}
        </span>
      </div>}
